...
const massages = [
    ...
    { name: 'Couples Swedish Massage', desc: 'Share a relaxing experience', durations: [{time: '60 min', price: 240}, {time: '90 min', price: 285}, {time: '120 min', price: 330}] },
    { name: 'Couples Deep Tissue', desc: 'Therapeutic deep tissue for two', durations: [{time: '60 min', price: 270}, {time: '90 min', price: 300}] }
];
...
const facials = [
    { name: 'Derma-planing', desc: 'Exfoliation treatment for smooth skin', price: 65 },
    ...
    { name: 'Hydration Facial', desc: 'Deep hydration treatment for dry skin', price: 110 },
    { name: 'Hyperpigmentation Facial', desc: 'Dark spot and pigmentation treatment', price: 120 },
    { name: 'Anti Aging', desc: 'Anti-aging treatment for youthful skin', price: 120 },
    ...
    { name: 'Couples Facial - 60min', desc: 'Share a rejuvenating facial experience', price: 240 },
    { name: 'Couples Facial - 90min', desc: 'Extended couples facial experience', price: 285 }
];

const facialTreatmentPlans = [
    { 
        name: 'Hydrafacial Signature Treatment Plan', 
        desc: 'Skincare package with FREE unlimited facial add-ons', 
        durations: [
            {time: '3 sessions', price: 465, savings: 30},
            {time: '6 sessions', price: 930, savings: 60},
            {time: '9 sessions', price: 1395, savings: 90},
            {time: '12 sessions', price: 1860, savings: 120}
        ],
        benefits: 'Includes: All facial enhancements, masks, serums, cold globes, gua sha, sonic massage, light therapy'
    },
    { 
        name: 'Hydrafacial Deluxe Treatment Plan', 
        desc: 'Enhanced skincare package with FREE unlimited facial add-ons', 
        durations: [
            {time: '3 sessions', price: 540, savings: 30},
            {time: '6 sessions', price: 1065, savings: 75},
            {time: '9 sessions', price: 1590, savings: 120},
    ...
    { 
        name: 'Hydrafacial Platinum Treatment Plan', 
        desc: 'Ultimate skincare package with FREE unlimited facial add-ons', 
        durations: [
            {time: '3 sessions', price: 630, savings: 30},
            {time: '6 sessions', price: 1245, savings: 75},
            {time: '9 sessions', price: 1860, savings: 120},
            {time: '12 sessions', price: 2475, savings: 165}
        ],
        benefits: 'Includes: All facial enhancements, masks, serums, cold globes, gua sha, sonic massage, light therapy'
    },
    { 
        name: 'Back Facial Treatment Plan', 
        desc: 'Back treatment package with FREE unlimited facial add-ons', 
        durations: [
            {time: '3 sessions', price: 285, savings: 15},
            {time: '6 sessions', price: 555, savings: 45},
            {time: '9 sessions', price: 825, savings: 75},
            {time: '12 sessions', price: 1095, savings: 105}
        ],
        benefits: 'Includes: All facial enhancements, masks, serums, specialized back treatments'
    },
    { 
        name: 'Acne Cleanse Treatment Plan', 
        desc: 'Acne treatment package with FREE unlimited facial add-ons', 
        durations: [
            {time: '3 sessions', price: 315, savings: 15},
            {time: '6 sessions', price: 615, savings: 45},
            {time: '9 sessions', price: 915, savings: 75},
            {time: '12 sessions', price: 1215, savings: 105}
        ],
        benefits: 'Includes: All facial enhancements, specialized acne treatments, extractions'
    }
];
...
const massageAddons = [
    ...
    { name: 'Sauna', desc: 'Relaxing sauna session', price: 35 },
...
function loadFacialChoices() {
    ...
    // Single Session Facials
    html += '<h3 class="section-header"><i class="fas fa-user"></i> Single Session Facials</h3>';
    html += '<div class="service-cards-grid">';
    
    facials.forEach((facial, idx) => {
        // Check if facial has multiple duration options
        if (facial.durations && facial.durations.length > 0) {
            html += `
                <div class="compact-service-card" data-service="facial" data-index="${idx}">
                    <div class="service-card-name">${facial.name}</div>
                    <div class="service-card-desc">${facial.desc}</div>
                    <select class="service-card-select" onchange="selectFacialFromCard('${facial.name}', this.value, event)">
                        <option value="">Select option...</option>
                        ${facial.durations.map(dur => `
                            <option value="${dur.time}|${dur.price}">${dur.time} - $${dur.price}</option>
                        `).join('')}
                    </select>
                </div>
            ...
            html += `
                <div class="compact-service-card" data-service="facial" data-index="${idx}" onclick="selectFacialFromCard('${facial.name}', ${facial.price}, event)">
                    <div class="service-card-name">${facial.name}</div>
                    <div class="service-card-desc">${facial.desc}</div>
                    <div class="choice-price" style="font-family: 'Cinzel', serif; color: #D4AF37; font-weight: 700; font-size: 1rem; margin-top: 0.5rem;">$${facial.price}</div>
                </div>
            `;
        }
    });
    ...
    // Facial Treatment Plans Section
    html += '<div class="section-divider"></div>';
    html += '<h3 class="section-header"><i class="fas fa-calendar-check"></i> Facial Treatment Plans</h3>';
    html += '<p style="font-family: \'Cinzel\', serif; color: #F4E5C2; font-size: 0.85rem; margin-bottom: 1rem; font-weight: 500;">Multi-session packages with FREE unlimited facial add-ons + BNPL available</p>';
    html += '<div class="service-cards-grid">';
    
    facialTreatmentPlans.forEach((plan, idx) => {
        html += `
            <div class="compact-service-card" data-service="facial-treatment-plan" data-index="${idx}">
                <div class="treatment-plan-badge">TREATMENT PLAN</div>
                <div class="service-card-name">${plan.name}</div>
                <div class="service-card-desc">${plan.desc}</div>
                <select class="service-card-select" onchange="selectFacialFromCard('${plan.name}', this.value, event)">
                    <option value="">Select package...</option>
                    ${plan.durations.map(dur => `
                        <option value="${dur.time}|${dur.price}">${dur.time} - $${dur.price}${dur.savings ? ' (Save $' + dur.savings + ')' : ''}</option>
                    `).join('')}
...
function selectFacialFromCard(name, priceOrValue, event) {
    // Check if this is a treatment plan with dropdown value
    if (typeof priceOrValue === 'string' && priceOrValue.includes('|')) {
        if (!priceOrValue) return;
        
        // Parse the value (format: "duration|price")
        const [duration, price] = priceOrValue.split('|');
        
        // Check if this is a facial treatment plan
        const isTreatmentPlan = name.includes('Treatment Plan');
        packageConfig.isTreatmentPlan = isTreatmentPlan;
        
        // Highlight the selected card
        const card = event.target.closest('.compact-service-card');
        document.querySelectorAll('.compact-service-card[data-service="facial-treatment-plan"]').forEach(c => c.classList.remove('selected'));
        card.classList.add('selected');
        ...
        // Call the selection function with duration info
        selectFacial(name, parseInt(price), duration, event);
    } else {
        // Single session facial
        const card = event.currentTarget;
        document.querySelectorAll('.compact-service-card[data-service="facial"]').forEach(c => c.classList.remove('selected'));
        card.classList.add('selected');
        
        selectFacial(name, priceOrValue, null, event);
    }
}
...
function loadAddonChoices() {
    ...
    // Determine which add-ons to show based on selections
    const hasMassage = packageConfig.massage !== null;
    const hasFacial = packageConfig.facial !== null;
    
    // Show massage add-ons if massage was selected
    if (hasMassage) {
        html += '<h3 class="section-header" style="font-family: \'Cinzel\', serif; color: #D4AF37; font-size: 1.1rem; font-weight: 700; margin-bottom: 1rem;"><i class="fas fa-spa"></i> Massage Add-Ons</h3>';
        html += '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem; margin-bottom: 2rem;">';
        html += massageAddons.map(addon => {
    ...
    // Show facial add-ons if facial was selected
    if (hasFacial) {
        html += '<h3 class="section-header" style="font-family: \'Cinzel\', serif; color: #D4AF37; font-size: 1.1rem; font-weight: 700; margin-bottom: 1rem;"><i class="fas fa-user"></i> Facial Add-Ons</h3>';
        html += '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem;">';
        html += facialAddons.map(addon => {
            const displayPrice = isFree ? 0 : addon.price;
            const priceText = isFree ? '<span style="color: #90EE90; font-weight: 700;">FREE</span>' : `+$${addon.price}`;
...
function selectFacial(name, price, duration, event) {
    // Animate selection
    ...
    setTimeout(() => {
        ...
        packageConfig.facialDuration = duration || '60 min session';
        
        // Check if it's a facial treatment plan and add note about benefits
        if (name.includes('Treatment Plan')) {
            packageConfig.notes = (packageConfig.notes ? packageConfig.notes + '\n\n' : '') + 
                'Facial Treatment Plan Benefits: FREE unlimited facial add-ons (masks, serums, eye treatments, cold globes, gua sha, sonic massage, light therapy, chemical peels). BNPL available (Afterpay, Klarna, Affirm).';
        }
    ...
    if (packageConfig.facial) {
        html += `
            <div class="summary-item">
                ...
                <div class="summary-item-details">${packageConfig.facialDuration || '60 min session'}</div>
            </div>
        `;
    }
